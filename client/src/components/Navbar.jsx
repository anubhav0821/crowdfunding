  ...
  return (
    ...
    justify-between mb-[35px] gap-6'>
      <div className='lg:flex-1 flex flex-row max-w-[458px] py-2 pl-4 pr-2 h-[52px] bg-[#1c1c24] rounded-[100px]'>
        <input type="text" placeholder='Search for campaingns'
      ...
      <div className='w-[40px] h-[40px] rounded-[10px] bg-[#2c2f32] flex justify-center items-center
            ...
            shadow-secondary py-4 ${!toggleDrawer ? '-translate-y-[100vh]' : 'transalate-y-0'} transation-all duration-700`}>
                <ul className='mb-4'>
                  {navlinks.map((link) => (
                    <li 
                    key={link.name}
                    className={`flex p-4 ${isActive === link.name && 'bg-[#3a3d43]'}`}
                    onClick={() => {
                    ...
                    >
                        ...
                        src={link.imgUrl}
                        alt={link.name}
                        className={`w-[24px] h-[24px] object-contain cursor-pointer ${isActive === link.name ? 'grayscale-0' : 'grayscale'}`}
                        />
                        <p className={`ml-[20px] font-epilogue font-semibold text-[14px] cursor-pointer
